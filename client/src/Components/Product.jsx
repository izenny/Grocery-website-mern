  ...
  return (
    <div className="m-1 flex h-full overflow-y-scroll no-scrollbar justify-around flex-wrap">
      {products.map((product) => (
        <div key={product.id} className="m-6 relative">
          <div className="w-[200px] h-[130px] p-2 bg-white shadow rounded-lg">
            <div className="flex justify-center flex-col items-center">
              <h2 className="text-2xl font-Merriweather">{product.productname}</h2>
          ...
          <div className="absolute w-[100px] h-[100px] rounded-xl top-[60px] left-[130px]">
            <img src={`../../public/ProductImages/${product.image}`} alt={product.productName} className="h-full w-full object-cover" />
          </div>
        </div>
      ))}
    </div>
